  ...
  for (int z = 0; z < im.channels(); z++)
    for (int y = 0; y < nHeight; y++)
      for (int x = 0; x < nWidth; x++)
      {
        ...
        // output is factor times bigger, source is 1/factor times bigger
        ys = round(1 / factor * y);
        xs = round(1 / factor * x);
...
{
  // output: vector of 2 images {luminance (single channel), chrominance (three channel)}
...
{
  // creates two images like the Spanish castle color illusion
