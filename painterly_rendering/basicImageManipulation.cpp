...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp)
{
  ...
  float tr = im.smartAccessor(xc, yf, z, clamp); // top right
  float bl = im.smartAccessor(xf, yc, z, clamp); // bottom left
  float br = im.smartAccessor(xc, yc, z, clamp); // bottom right

  // compute the interpolations on the top and bottom, and left to right
  ...
  for (int z = 0; z < im.channels(); z++)
    for (int y = 0; y < nHeight; y++)
      for (int x = 0; x < nWidth; x++)
      {
        ...
        ys = 1 / factor * y;
        xs = 1 / factor * x;
        out(x, y, z) = interpolateLin(im, xs, ys, z, true);
      }
  return out;
}
...
std::vector<Image> lumiChromi(const Image &im)
{
  ...
  // Stack luminance and chrominance in the output vector
...
Image grayworld(const Image &im)
{
  ...
  // The mean value of the green channel is taken as reference.
